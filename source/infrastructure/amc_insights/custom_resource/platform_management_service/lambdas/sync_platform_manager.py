...
logger = Logger(service='Sync platform manager', level="INFO")

PLATFORM_MANAGER_DIR = "platform_manager"
...
def on_create_or_update(event, _) -> None:
    """
    This function handles the move of platform manager to S3 artifacts
    """
    ...
    try:
        ...
        raise err
    logger.info(
        f'Move platform manager to S3 artifacts {resource_properties["artifacts_bucket_name"]} {resource_properties["artifacts_key_prefix"]}')


@helper.delete
def on_delete(event, _):
    """
    This function is responsible for removing platform manager from the s3 artifact bucket.
    """
    ...
    try:
        ...
        logger.error(err)
        raise err
    logger.info("Platform manager is deleted.")


def delete_bucket_contents(resource_properties) -> None:
    s3_client = get_service_client("s3")
    for file in list_files_to_sync(PLATFORM_MANAGER_DIR):
