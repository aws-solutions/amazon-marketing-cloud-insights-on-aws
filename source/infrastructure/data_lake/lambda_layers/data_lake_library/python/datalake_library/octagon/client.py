...
from aws_solutions.core.helpers import get_service_resource, get_service_client, get_session
import os
...
class OctagonClient:
    ...
    def with_configuration_instance(self, instance: str, resource_prefix):
    ...
    def build(self):
        ...
        if self.run_in_fargate:
            ...
            self.logger.info("Run datalake_library in AWS Lambda")
            pass # NOSONAR

        else:
            get_session(profile_name=self.profile, region_name=self.region)

        self.account_id = get_service_client("sts").get_caller_identity().get("Account")
        self.dynamodb = get_service_resource("dynamodb")
        self.sns = get_service_client("sns")
        self.config = ConfigParser(self.configuration_file, self.configuration_instance, self.resource_prefix)
