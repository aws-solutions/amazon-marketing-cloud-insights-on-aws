...
class ConfigParser:
    def __init__(self, config_file, instance, resource_prefix):
        ...
        for config_instance in config_dict["configuration_instances"]:
            if config_instance["instance"] == instance:
                self.check_config_instance_tables(config_instance)
    ...
    def check_config_instance_tables(self, config_instance):
        for ti in config_instance["tables"]:
                    if "object" in ti.keys() and "table_name" in ti.keys():
                        table_name_in_config_file = ti["table_name"]
                        table_info = ConfigTableInfo(
                            dynamo_table_name=f"{table_name_in_config_file}-{self.resource_prefix}",
