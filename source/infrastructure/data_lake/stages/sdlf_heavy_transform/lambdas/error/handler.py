...
def lambda_handler(event, _):
    ...
    metrics.Metrics(METRICS_NAMESPACE, STACK_NAME, logger).put_metrics_count_value_1(metric_name="SdlfHeavyTransformError")
    try:
        if isinstance(event, str):
            event = json.loads(event)
        sqs_config = SQSConfiguration(
            resource_prefix, event['body']['team'], event['body']['dataset'], event['body']['pipeline_stage'])
