...
logger = Logger(service='Sync Glue Scripts', level="INFO")

GLUE_SCRIPT_DIR = "sdlf_heavy_transform"
...
def on_create_or_update(event, _) -> None:
    """
    This function handles the move of Glue Scripts to S3 artifacts
    """
    ...
    try:
        ...
        raise err
    logger.info(
        f'Move Glue Scripts to S3 artifacts {resource_properties["artifacts_bucket_name"]} {resource_properties["artifacts_key_prefix"]}')
...
def upload_bucket_contents(resource_properties) -> None:
    ...
    for file in list_files_to_sync(GLUE_SCRIPT_DIR):
...
def get_bucket_name_and_key(resource_properties, file):
    ...
    object_key = f"{artifacts_key_prefix}{file.removeprefix('sdlf_heavy_transform/')}"
