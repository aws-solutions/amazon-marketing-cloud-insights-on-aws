...
#
# PURPOSE:
#  Run functional tests for the sp report dataset
#
# PRELIMINARY:
...
#########################
echo "------------------------------------------------------------------------------"
echo "** Initializing functional test for Sp Report Dataset **"
echo "------------------------------------------------------------------------------"
...
export TEST_SP_REPORT_DATASET=$(aws cloudformation describe-stacks --stack-name "$stack_name" --region "$region" --profile "$profile" --query "Stacks[].Parameters[?ParameterKey=='ShouldDeployDataLake'].ParameterValue" --output text)
if [[ "$TEST_SP_REPORT_DATASET" == "Yes" ]]; then
  ...
  echo "Sp Report Dataset has not been deployed. There is nothing to test"
  die 1
fi
...
export MOCK_DATA="sp-report-mock-data.json"
...
pytest python/test_sp_report.py -s -W ignore::DeprecationWarning -p no:cacheproviders -vv
