...
# ###############################################################################
# PURPOSE:
#   * Unit test for Selling Partner Reporting/Lambdas/GetReportStatus handler.
# USAGE:
#   ./run-unit-tests.sh --test-file-name reporting_microservices_tests/selling_partner_reporting_tests/lambdas/test_GetReportStatus.py
###############################################################################
...
def test_handler_success(mock_selling_partner_api_response, mock_selling_partner_api):
    from reporting_microservices.selling_partner_reporting.reporting_service.lambdas.GetReportStatus.handler import handler
    ...
    mock_mock_selling_partner_api_instance.get_report_status.return_value = MagicMock(
        response={'reportDocumentId': "test-document-id"})

    result = handler(mock_event, mock_context)

    assert result['reportDocumentId'] == "test-document-id"
    mock_selling_partner_api.assert_called_once_with(region="North America", auth_id=None)
    mock_mock_selling_partner_api_instance.get_report_status.assert_called_once_with(report_id=123456)
