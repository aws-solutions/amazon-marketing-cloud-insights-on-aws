...
# USAGE:
#   ./run-unit-tests.sh --test-file-name amc_insights_tests/microservices/workflow_manager_service/lambdas/test_CreateWorkflow.py
###############################################################################
...
def test_handler(_mock_imports):
    from amc_insights.microservices.workflow_manager_service.lambdas.CreateWorkflow.handler import handler
    ...
    with (
        ...
        with patch("urllib3.PoolManager"):
            ...
            amc_response_mock.return_value.create_workflow.return_value = MagicMock(success=True, response={})
            ...
            amc_response_mock.return_value.create_workflow.return_value = MagicMock(success=False, response={})
            test_event["workflowRequest"] = {"workflowDefinition": {"workflowId": 12345}, 'workflowId': 12345}
            event = handler(test_event, None)
            assert event == {'workflowId': 12345, 'customerConfig': {'Item': {}},
                             'workflowRequest': {'workflowDefinition': {'workflowId': 12345}, 'workflowId': 12345},
                             }
