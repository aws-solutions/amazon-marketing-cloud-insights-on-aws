...
# ###############################################################################
# PURPOSE:
#   * Unit test for MicroServices/WFM/DeleteWorkflow handler.
# USAGE:
#   ./run-unit-tests.sh --test-file-name amc_insights_tests/microservices/workflow_manager_service/lambdas/test_DeleteWorkflow.py
###############################################################################
...
def test_handler(_mock_imports):
    from amc_insights.microservices.workflow_manager_service.lambdas.DeleteWorkflow.handler import handler
    ...
    test_event = {
        ...
        "workflowRequest": {"workflowDefinition": {"workflowId": 12345}, "workflowId": 12345}
    }
    ...
    with (
        ...
        with patch("urllib3.PoolManager"):
            ...
            amc_interface_mock.return_value.delete_workflow.return_value = MagicMock(success=True, response={})
            ...
            assert event == {'workflowId': 12345, 'customerConfig': {'Item': {}}, 'workflowRequest': {'workflowDefinition': {'workflowId': 12345}, 'workflowId': 12345}}
