...
# ###############################################################################
# PURPOSE:
#   * Unit test for MicroServices/WFM/CheckWorkflowExecutionStatus handler.
# USAGE:
#   ./run-unit-tests.sh --test-file-name amc_insights_tests/microservices/workflow_manager_service/lambdas/test_CheckWorkflowExecutionStatus.py
###############################################################################
import os
...
def test_handler(lambda_event, lambda_context, _mock_imports):
    from amc_insights.microservices.workflow_manager_service.lambdas.CheckWorkflowExecutionStatus.handler import handler
    event = handler(lambda_event, lambda_context)
    assert event == {
        'customerConfig': {
            'Item': {}
                            },
        'executionRequest': {'workflowExecutionId': '12345'}}
